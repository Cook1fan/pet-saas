        ...
        <el-table-column label="价格" width="160">
          <template #default="{ row }">
            <div class="price-info">
              ...
              <div class="seckill-price">¥{{ formatPrice(row.price) }}</div>
            </div>
          </template>
        </el-table-column>
...
import {
  ...
  type ActivityInfoVO,
  type ActivityQuery,
  type CreateSeckillActivityReq,
  type ActivityData
} from '@/api/groupActivity'
import { getGoodsList, type Goods, getGoods } from '@/api/inventory'
...
const queryForm = reactive<ActivityQuery>({
  ...
  title: '',
  status: undefined,
  type: ACTIVITY_TYPE.SECKILL
})

// 对话框表单
const dialogForm = reactive<Partial<CreateSeckillActivityReq>>({
  title: '',
  goodsId: undefined,
  price: 0,
  originPrice: 0,
    ...
    if (valid) {
      ...
      try {
        if (isEdit.value && currentActivity.value) {
          await updateSeckillActivity(
            currentActivity.value.id,
            dialogForm as CreateSeckillActivityReq
          )
          ElMessage.success('更新成功')
        } else {
          await createSeckillActivity(dialogForm as CreateSeckillActivityReq)
